...
export default function Auction() {
  ...
  async function init() {
    ...
    const channel = supabase.channel(`auction-${mem.league_id}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'auction_bids', filter: `league_id=eq.${mem.league_id}` },
        async (payload) => {
  ...
  async function refreshData(mem) {
    const { data: allMem } = await supabase.from('league_members').select('*, profiles(*)').eq('league_id', mem.league_id)
    setMembers(allMem || [])
    ...
    const unsold = (allPlayers || []).filter(p => !soldIds.has(p.id))
    setPlayers(unsold)
    ...
    if (latestBids?.length > 0) {
      ...
      // Only show if this player is still unsold
      if (!soldIds.has(latest.player_id)) {
  ...
  async function placeBid(amount) {
    ...
    setBidding(true)
    await supabase.from('auction_bids').insert({ league_id: league.id, player_id: currentPlayer.id, bidder_id: profile.id, amount })
    setBidding(false)
    startTimer()
  }
  ...
  async function sellPlayer() {
    ...
    const { data: alreadySold } = await supabase.from('squad').select('id').eq('player_id', currentPlayer.id).eq('league_id', league.id).maybeSingle()
    if (alreadySold) { setSoldMsg('Player already sold!'); setCurrentPlayer(null); setCurrentBid(0); setCurrentBidder(null); setBidHistory([]); return }
    ...
    const { error } = await supabase.from('squad').insert({
      ...
      bought_price: currentBid
    })
    if (error) { setSoldMsg('Error: ' + error.message); return }

    // Deduct purse from winner
    const { data: winnerMem } = await supabase.from('league_members').select('purse_remaining').eq('league_id', league.id).eq('user_id', currentBidder.id).single()
if (winnerMem) {
  await supabase.from('league_members').update({ purse_remaining: winnerMem.purse_remaining - currentBid }).eq('league_id', league.id).eq('user_id', currentBidder.id)
}
  ...
  async function startBiddingOnPlayer(player) {
    ...
    setBidHistory([])
    await supabase.from('auction_bids').insert({ league_id: league.id, player_id: player.id, bidder_id: profile.id, amount: player.base_price })
    startTimer()
  }
  ...
  if (loading) return <div style={{ display:'flex',alignItems:'center',justifyContent:'center',height:300 }}><div style={{ fontFamily:'Rajdhani',fontSize:20,color:'var(--gold)' }}>Loading auction...</div></div>
  if (!league) return <div style={{ padding:40,textAlign:'center',color:'var(--muted)' }}>Join a league first from the Dashboard.</div>
    ...
    <div>
      <div className="fade-in" style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:24 }}>
        <div>
          ...
          <div style={{ color:'var(--muted)',fontSize:14,marginTop:2 }}>{players.length} players remaining · ₹120 Cr purse each</div>
        </div>
        <div style={{ display:'flex',alignItems:'center',gap:8 }}>
          <div style={{ display:'flex',alignItems:'center',gap:6,background:'rgba(232,69,69,0.1)',border:'1px solid rgba(232,69,69,0.25)',borderRadius:20,padding:'6px 14px' }}>
            <div className="live-dot" /><span style={{ fontSize:12,fontWeight:700,color:'var(--red)',letterSpacing:0.5 }}>LIVE</span>
          </div>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'1fr 300px',gap:16 }}>
        <div>
          ...
          {/* SOLD button - admin only, shows when timer hits 0 */}
          ...
          {member?.is_admin && (
            <div style={{ marginTop:20 }}>
              ...
              <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,maxHeight:400,overflowY:'auto' }}>
                {players.map(p => (
                  <div key={p.id} onClick={() => startBiddingOnPlayer(p)} style={{ background:'var(--navy2)',border:'1px solid var(--border)',borderRadius:10,padding:'10px 14px',cursor:'pointer',transition:'all 0.15s',display:'flex',alignItems:'center',gap:10 }}
        ...
        <div>
          ...
          <div className="card" style={{ padding:16 }}>
            ...
            {bidHistory.length===0 && <div style={{ color:'var(--muted)',fontSize:13,textAlign:'center',padding:'12px 0' }}>No bids yet</div>}
            {bidHistory.map((b,i) => (
